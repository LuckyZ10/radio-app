...
class _ListenerLiveScreenState extends State<ListenerLiveScreen> {
  ...
  void _setupListeners() {
    ...
    _socketService.onRoomEnded = (data) {
      if (mounted && data['roomId'] == widget.room['id']) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Broadcast has ended')));
    ...
    _socketService.onCallAnswered = (data) {
      if (mounted) {
        ...
        setState(() { _isOnCall = true; _isRequestingCall = false; });
      }
    };
    ...
    _webrtcService.onError = (error) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Call error: $error')));
  ...
  Future<void> _requestCall() async {
    ...
    if (broadcasterSocketId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Broadcaster is not available')));
      return;
    }
    try {
      setState(() => _isRequestingCall = true);
      await _webrtcService.initialize();
      await _webrtcService.createCall(broadcasterSocketId, 'Listener');
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to request call: $e')));
        setState(() => _isRequestingCall = false);
      }
    }
  }
  ...
  void _endCall() {
    final broadcasterSocketId = widget.room['broadcasterSocketId'];
    if (broadcasterSocketId != null) _socketService.endCall(broadcasterSocketId);
    _webrtcService.endCall();
    setState(() { _isOnCall = false; _isRequestingCall = false; });
  }
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    Text('$_listenerCount listening', style: TextStyle(color: Colors.grey[600])),
                  ],
                ),
                ...
                if (_isOnCall) ...[
                  Container(
                    ...
                    child: const Row(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      SizedBox(width: 8),
                      Text('Connected to broadcaster', style: TextStyle(fontWeight: FontWeight.bold)),
                    ]),
                  ),
                  const SizedBox(height: 16),
                  ElevatedButton.icon(
                  ...
                  const SizedBox(height: 16),
                  Text('Requesting to connect...', style: TextStyle(color: Colors.grey[600])),
                ] else ...[
                  ElevatedButton.icon(
                  ...
                  const SizedBox(height: 8),
                  Text('Request a live conversation with the broadcaster', style: TextStyle(color: Colors.grey[600], fontSize: 12)),
                ],
                const SizedBox(height: 32),
                Row(
                  mainAxisAlignment: MainAxisAlignment.center,
